clear all;
% we are solving x'=f(x,t) using Improved Euler
...
xsF=zeros(1,N+1); % saves all values of x for Improved Euler
xsF(1)=x0;
 
%xold=x0;
 
for n=1:N
    xsF(n+1)=xsF(n)+(h/2)*((-ts(n)*xsF(n)+0.1*xsF(n)^3)+(-ts(n+1)*(xsF(n)+(1/2)*(-ts(n)*xsF(n)+0.1*xsF(n)^3))+0.1*((xsF(n)+(1/2)*(-ts(n)*xsF(n)+0.1*xsF(n)^3))^3))); % in parentheses is f(x,t) -- Improved Euler
    % Output at each step
    disp(['Improved Euler predicts the value x(' num2str(ts(n+1)) ')=' num2str(xsF(n+1))])
end
